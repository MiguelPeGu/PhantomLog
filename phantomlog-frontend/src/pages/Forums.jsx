      ...
      <div className="max-1200">
        <div className={`grid-3 loading-fade${fading ? ' is-loading' : ''}`}>
          {loading && forums.length === 0 ? (
            ...
            <div className="text-center p-100 border-dashed" style={{ gridColumn: '1/-1' }}>
              NO SE HAN ENCONTRADO ARCHIVOS CON ESTE PATRÓN.
            </div>
          ) : (
            forums.map(f => (
              <div key={f.id} className="horror-card column">
                <Link to={`/forums/${f.id}`} className="no-underline flex-1">
                  <div className="card-image-box">
                    <ShimmerImage src={f.image_url} alt={f.title} objectFit="cover" />
                  </div>
